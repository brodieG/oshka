...
summr <- function(.data, ...) {
  exps.sub <- recsub(substitute(list(...)), .data, parent.frame())

  splits <- eval(attr(.data, ".GRP"), .data, parent.frame())
  dat.split <- split(.data, splits)
  res.list <- replicate(
    length(dat.split), vector("list", length(exps.sub) - 1), simplify=FALSE
  )

  for(i in seq_along(dat.split))
    for(j in tail(seq_along(exps.sub), -1L))
      res.list[[i]][[j]] <- eval(exps.sub[[j]], dat.split[[i]], parent.frame())
