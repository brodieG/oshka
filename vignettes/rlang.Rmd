...
summr <- function(.data, ...) {
  ...
  grps.names <- group_names(grps)

  dat.split <- unname(split(.data, splits, drop=TRUE))
  grp.split <- unname(
    split(setNames(as.data.frame(splits), grps.names), splits, drop=TRUE))
  ...
  lens <- lapply(res.list, lengths, integer(length(splits)))
  grp.lens <- do.call(pmax, lens) * (do.call(pmin, lens) != 0)

  res.list.r <- lapply(
    c(grp.split, res.list),
    function(x) unlist(Map(recycle, x, grp.lens))
  )
