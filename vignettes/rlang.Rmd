...
In this case we are forwarding NSE expressions (`i`, `j`, and `by`) to our NSE
`summarize_r_l`, `group_r_l`, and `filter_r` functions.  These are the same
functions that implement the ersatz `dplyr`.

One trade off we make with `recsub` is that we do not capture the environment of
the NSE expressions.  This means that when forwarding the expressions we need to
make sure they are evaluated in the correct frame.  Here we do so by constructing
the call with `bquote` and evaluating it in the parent frame.  Note we leave the
functions unquoted because we want them to resolve in the environment of
`[.super.df`, not in the parent frame.

While this is a little extra work, it is implemented with the base R functions
`sustitute`, `eval`, and `bquote`, and it is easy for the function users to use:
