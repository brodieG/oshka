...
subset3 <- function(x, subset, select, drop = FALSE, ...) {
  eval(
    bquote(
      .(base::subset)(
        .(x),
        .(recsub(substitute(subset), x, parent.frame())),
        .(recsub(substitute(select), x, parent.frame())),
        .(drop), `...`=.(list(...))
    ) ),
    parent.frame()
  )
...
One limitation of `recsub` is that as with `eval` you are responsible for
ensuring the evaluation context is correct.
