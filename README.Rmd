...
Our main objective with `recsub` is a simple R-semantic interface to
programmable NSE at the command line.  `recsub` does not simplify
forwarding NSE arguments to other NSE functions, although that problem can be
solved with base R methods:
...
We use `bquote` to assemble our substituted call and `eval` and evaluate in
the correct frame.  This requires some work from the programmer, but the user
reaps the benefits:
...
The expressions are unquoted and evaluated in a natural way.

The forwarding should be robust to unusual evaluation:

```{r}
col <- quote(I_dont_exist)
sub.a <- quote(stop("all hell broke loose"))
sub.b <- quote(stop("all hell broke loose"))

local({
  col <- quote(Petal.Length)
  sub.a <- quote(Species == 'setosa')
  sub.b <- quote(Species == 'versicolor')
  subs <- list(sub.a, sub.b)

  lapply(
    subs,
    function(x) subset3(iris, x & col > 4.9, col:Petal.Length)
  )
})
```
